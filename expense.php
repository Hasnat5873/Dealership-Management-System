...
if ($role === 'User') {
        ...
        if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
            ...
            try {
                if (isset($_POST['add'])) {
                    ...
                    $date_error = validate_date($expense_date);

                    if ($desc_error || $amount_error || $date_error) {
                        $errorMsg = implode(" ", array_filter([$desc_error, $amount_error, $date_error]));
                    } else {
                        $stmt = $conn->prepare("INSERT INTO general_expenses (user_id, description, amount, expense_date) 
                                                VALUES (:user_id, :description, :amount, :expense_date)");
                        $stmt->execute([
                            'user_id' => $user_id,
                    ...
                    if ($id <= 0) {
                        $errorMsg = "Invalid expense ID.";
                    } elseif ($desc_error || $amount_error || $date_error) {
                        $errorMsg = implode(" ", array_filter([$desc_error, $amount_error, $date_error]));
                    } else {
                        $stmt = $conn->prepare("UPDATE general_expenses 
                        ...
                        if ($stmt->rowCount() > 0) {
                            ...
                            $errorMsg = "No expense found or you are not authorized.";
                        }
                    }
                    ...
                    if ($id <= 0) {
                        ...
                        $stmt = $conn->prepare("DELETE FROM general_expenses WHERE id = :id AND user_id = :user_id");
    ...
    try {
        $stmt = $conn->prepare("SELECT * FROM general_expenses WHERE user_id = :user_id ORDER BY expense_date DESC");
        $stmt->execute(['user_id' => $user_id]);
        $expenses = $stmt->fetchAll(PDO::FETCH_ASSOC);
    } catch (PDOException $e) {
        $errorMsg = "Error fetching expenses: " . htmlspecialchars($e->getMessage());
    }
} else {
    $errorMsg = "Access denied. Only Users can view or manage expense data.";
    $expenses = [];
}
?>
...
<head>
    ...
    <title>General Expense Management - Elite Dashboard</title>
...
<body class="min-h-screen text-gray-100 font-inter">
    ...
    <header class="bg-gradient-to-r from-emerald-800 to-teal-700 text-white py-8 px-8 shadow-2xl">
        <div class="container mx-auto flex items-center justify-between pl-64">
            <div class="flex items-center space-x-4">
                ...
                <h1 class="text-4xl font-bold gradient-text"> Expense Management</h1>
            </div>
    ...
    <div class="container mx-auto mt-10 px-6 flex">
        ...
        <main class="flex-1 ml-64 p-10" role="main">
            ...
            <div class="glass-card p-10 rounded-2xl fade-in">
                <?php if ($role === 'User'): ?>
                    <form class="add-form mb-6 grid grid-cols-1 md:grid-cols-2 gap-6" method="POST" id="addExpenseForm">
                        ...
                        <div class="form-group">
                            <label for="expense_date" class="text-lg font-semibold gradient-text mb-2">Date</label>
                            <input type="date" name="expense_date" id="expense_date" value="<?php echo date('Y-m-d'); ?>" 
                                   class="glass-card p-4 rounded-lg border-none text-gray-100 placeholder-gray-400 focus:ring-2 focus:ring-emerald-500" required />
                        </div>
                        <button type="submit" name="add" class="bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white p-4 rounded-lg col-span-1 md:col-span-2 transition-all duration-300 shadow-lg">
                            <i class="fa-solid fa-plus mr-3"></i> Add Expense
                        </button>
                    </form>
                    <div class="table-container">
                        <h3 class="text-xl font-semibold gradient-text mb-6">Expense List</h3>
                        <table class="w-full text-left">
                            ...
                            <tbody>
                                <?php if ($expenses): ?>
                                    <?php foreach ($expenses as $e): ?>
                                        <tr class="border-b border-gray-700/50">
                                            <td class="p-5" data-label="ID"><?php echo htmlspecialchars($e['id']); ?></td>
                                            <td class="p-5" data-label="Description"><?php echo htmlspecialchars($e['description']); ?></td>
                                            <td class="p-5" data-label="Amount (৳)"><?php echo number_format($e['amount'], 2); ?></td>
                                            <td class="p-5" data-label="Date"><?php echo htmlspecialchars($e['expense_date']); ?></td>
                                            <td class="p-5 action-buttons" data-label="Actions">
                                                <button class="update-btn bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-3 rounded-lg transition-all duration-200 shadow" 
                                                        ...
                                                        data-amount="<?php echo htmlspecialchars($e['amount']); ?>" 
                                                        data-date="<?php echo htmlspecialchars($e['expense_date']); ?>" 
                                                        title="Edit Expense">
                                                    <i class="fa-solid fa-pen"></i>
                                                </button>
                                                <form method="POST" onsubmit="return confirm('Are you sure you want to delete this expense?');" class="inline-block">
                                                    <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
                                                    <input type="hidden" name="id" value="<?php echo htmlspecialchars($e['id']); ?>">
                                                    <button type="submit" name="delete" class="bg-red-600 hover:bg-red-700 text-white px-4 py-3 rounded-lg transition-all duration-200 shadow" title="Delete Expense">
                                ...
                                <?php else: ?>
                                    <tr>
                                        <td colspan="5" class="p-5 text-center">No expense records found.</td>
                                    </tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
                ...
                <?php else: ?>
                    <div class="text-center">
                        ...
                        <p class="text-gray-400">Only Users can view or manage expense data.</p>
                    </div>
                <?php endif; ?>
            </div>
        </main>
    ...
    <div class="modal-overlay fixed inset-0 flex items-center justify-center z-[1000] hidden" id="updateModal" aria-hidden="true" role="dialog">
        <div class="modal p-10 rounded-2xl max-w-lg w-full">
            ...
            <form method="POST" id="updateExpenseForm" class="space-y-6">
                <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
                <input type="hidden" name="id" id="updateExpenseId">
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="expense_date" id="updateDate" class="w-full p-4 rounded-lg" required>
                </div>
    ...
    <script>
        ...
        const updateExpenseId = document.getElementById('updateExpenseId');
        ...
        document.querySelectorAll('.update-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                updateExpenseId.value = btn.getAttribute('data-id');
        ...
        // Client-side Validation for Add Expense Form
        if (addExpenseForm) {
        ...
        if (updateForm) {
            updateForm.addEventListener('submit', (e) => {
                ...
                const expense_date = updateDate.value;
